...
function Pillar({ label, description }: PillarProps) {
  return (
    <div className="flex gap-4 rounded-[1.1rem] border-2 border-line bg-surface-2 p-5 shadow-[0_5px_0_var(--shadow-color)]">
...
function FlowNode({ children, accent = false }: { children: React.ReactNode; accent?: boolean }) {
  return (
    <div className={`flex flex-col items-center justify-center rounded-2xl border-2 border-line px-4 py-3.5 text-center shadow-[0_5px_0_var(--shadow-color)] ${accent ? "bg-ink text-background" : "bg-surface-2 text-ink"}`}>
      {children}
    </div>
  );
}
...
function DataFlowVisual() {
  return (
    <m.div
      ...
      className="overflow-hidden rounded-[1.7rem] border-2 border-line bg-surface-3 p-6 shadow-[0_8.4px_0_var(--shadow-color)] sm:p-8"
    >
      {/* Header */}
